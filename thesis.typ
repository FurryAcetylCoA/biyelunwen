...
2010年，David Patterson等人出于教学目的，在仔细评估了市面上各类指令集后，发现它们都存在设计上的不足。
决定重头开始，吸取指令集架构领域25年期间的经验教训。设计了一款新的指令集架构—— RISC-V。
...
如 @fig:circle 所示。芯片行业的敏捷开发通常由高级语言写成的软件模拟器开始。软件仿真器的灵活性允许工程师快速尝试各种设计方案。
方案完成后，设计被迁移到现场可编程门阵列（FPGA）上。FPGA可以以高速和高准确性允许工程师在这一步运行完整的操作系统与测试程序。
并与客户探讨设计是否满足需求。工程师可以快速在软件模拟器上根据客户反馈调整设计，并再次进入FPGA流程。

设计完成后，可以通过电子设计自动化（EDA）工具对芯片电路的面积、功耗和时序进行细致的分析。
出现指标不满足时，前后端团队可以及时沟通，继续优化设计。
...
FIRRTL随后被输入硬件编译框架(Hardware Compiler Framework) CIRCT @circt2021。
经过多次递降变换（lowering transformations） @mlir2021，逐级去除高层次的抽象，最终输出优化后的底层RTL代码（如Verilog）。
使用HCF的好处之一是其内置了大量的优化器，可进行常量传播、共用表达式合并、不可达代码消除或向ASIC与FPGA提供针对性的优化#cite(<8203780>) 。
...
本文将首先实现一个单发射，非流水线，顺序执行的基本RISC-V 32E【格式是这样吗？】处理器微架构。
